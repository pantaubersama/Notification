module Subscribers
  class Badge < ApplicationSubscribe
    ...
    def work(params)
      ...
      elsif params[:notif_type].eql?("profile") && params[:event_type].eql?("profile")
        logger.info " --event_type: profile"
        BadgeService.new.when_receive_badge_profile(params[:receiver_id], params[:badge_id])

      elsif params[:notif_type].eql?("profile") && params[:event_type].eql?("relawan")
